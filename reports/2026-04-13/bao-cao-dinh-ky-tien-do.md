...
│          │                                       │          │ 4 pretrained models  │
├──────────┼───────────────────────────────────────┼──────────┼──────────────────────┤
│ 5-6      │ Tracking, đếm xe                      │ ⚠ 80%    │ SORT tracker: xong  │
│          │                                       │          │ Đếm theo camera     │
│          │                                       │          │ direction: dự kiến  │
├──────────┼───────────────────────────────────────┼──────────┼──────────────────────┤
...
│          │                                       │          │ OCR 2-row VN: xong  │
├──────────┼───────────────────────────────────────┼──────────┼──────────────────────┤
│ 9-10     │ Hậu xử lý OCR, đánh giá lỗi          │ ⚠ 60%    │ Baseline 50 ảnh: có │
│          │                                       │          │ Full 3,731 ảnh: chưa│
│          │                                       │          │ Regex biển VN: chưa │
├──────────┼───────────────────────────────────────┼──────────┼──────────────────────┤
...
│          │                                       │          │ 56/56 tests pass    │
├──────────┼───────────────────────────────────────┼──────────┼──────────────────────┤
│ 13-14    │ Dashboard realtime/thống kê           │ ⚠ 80%    │ Events list: OK     │
│          │                                       │          │ Các mục khác: chưa  │
│          │                                       │          │ test kỹ trên browser│
├──────────┼───────────────────────────────────────┼──────────┼──────────────────────┤
│ 15       │ Tích hợp, tối ưu, kiểm thử           │ ✅ Xong   │ E2E demo hoạt động  │
│          │                                       │          │ Test 14/16 pass     │
├──────────┼───────────────────────────────────────┼──────────┼──────────────────────┤
│ 16       │ Viết báo cáo, slide, demo             │ 🔄 Đang  │ Báo cáo .md + .docx │
│          │                                       │          │ 11 hình đã chụp/vẽ │
│          │                                       │          │ Slide: chưa         │
└──────────┴───────────────────────────────────────┴──────────┴──────────────────────┘

Nhận xét: Backend (tuần 11-12) và Dashboard (tuần 13-14) hoàn thành sớm hơn kế hoạch.
Tích hợp E2E hoạt động: video → AI detect → gửi backend → hiển thị dashboard.
Còn cần: đếm xe theo camera direction, full OCR eval, test dashboard kỹ hơn, slide bảo vệ.
Cập nhật lần cuối: 14/04/2026, dựa trên test logs tại test_plans_and_reports/.
...
3.1.3. Đếm phương tiện theo hướng
...
├───┼──────────────────────────────────────┼──────────────┼──────────┤
│ 1 │ Full OCR evaluation (3,731+ ảnh)     │ Ch.3.2, Ch.4 │ ~5 phút  │
│ 2 │ Regex hậu xử lý biển VN             │ Ch.3.2       │ ~1 giờ   │
│ 3 │ Char mapping (0↔O, 1↔I, 5↔S)       │ Ch.3.2       │ ~1 giờ   │
│ 4 │ Đếm xe line-crossing                │ Ch.3.1       │ ~3 giờ   │
│ 5 │ Screenshot dashboard cho báo cáo    │ Ch.3.3, Ch.4 │ ~30 phút │
│ 6 │ Vẽ diagram (kiến trúc, ERD, seq)    │ Ch.2         │ ~2 giờ   │
└───┴──────────────────────────────────────┴──────────────┴──────────┘
...
│ # │ Việc                                 │ Chương       │ Ước tính │
├───┼──────────────────────────────────────┼──────────────┼──────────┤
│ 7 │ Lưu snapshot crop biển số           │ Ch.3.3       │ ~2 giờ   │
│ 8 │ Benchmark FPS: GPU vs CPU           │ Ch.4         │ ~30 phút │
│ 9 │ Thêm video test đa dạng            │ Ch.4         │ ~1 giờ   │
│10 │ Viết phần lý thuyết (tìm paper)    │ Ch.1         │ ~5 giờ   │
└───┴──────────────────────────────────────┴──────────────┴──────────┘
...
9.2. Phương hướng cải thiện OCR accuracy

OCR exact match hiện tại: 33.3%. Chúng em dự kiến cải thiện bằng:
  • Regex post-processing (match format biển số VN: XXY-XXXXX hoặc XXY-XXX.XX)
  • Bảng char mapping cho ký tự dễ nhầm (0↔O, 1↔I, 5↔S, 8↔B)
  • Padding crop biển số trước OCR (thêm 10-15% border)

Xin thầy cho ý kiến nếu có đề xuất thêm phương pháp nào khác.

9.3. Phương hướng xác định chiều vào/ra

Đồ án thiết kế theo mô hình 1 luồng camera cho mỗi chiều. Khi lắp đặt thực
tế, mỗi camera được cấu hình chế độ cố định: camera chiều VÀO hoặc camera
chiều RA. Hệ thống dựa vào cấu hình camera để xác định hướng xe, không cần
thuật toán line-crossing hay zone-based counting.

Khi triển khai mở rộng nhiều cổng, mỗi cổng sẽ có 1-2 camera với chế độ
tương ứng, quản lý qua bảng `cameras` trong cơ sở dữ liệu (trường `gate_type`
và cấu hình direction).

Xin thầy cho ý kiến nếu cần bổ sung thêm phương pháp nào khác.

9.4. Dự kiến bổ sung chức năng trong thời gian tới

Chúng em dự kiến bổ sung các chức năng sau:
  ...
  • Hiển thị stream URL camera trên dashboard để giám sát trực tiếp
    (việc thêm/sửa/xóa camera sẽ thực hiện trực tiếp trên database,
    không cần giao diện quản trị riêng)

Xin thầy cho ý kiến nếu có chức năng nào khác cần thiết hơn cho bài toán.


Xem file riêng: reports/2026-04-13/plan-chup-anh.md
