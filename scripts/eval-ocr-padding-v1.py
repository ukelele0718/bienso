"""OCR eval experiment v1: Add 10% padding to plate crop before OCR.

Hypothesis: LP_detector crops too tight, missing edge characters.
Adding 10% padding on each side may recover missed chars.

Usage:
    python scripts/eval-ocr-padding-v1.py --limit 500
    python scripts/eval-ocr-padding-v1.py              # full test split
"""
...
PADDING_FRACTION = 0.10  # 10% padding on each side
...
def run_evaluation(limit: int | None = None) -> dict:
    ...
    print(f"[eval-v1] Variant: 10% padding on detector crop")
    print(f"[eval-v1] Loading models...")
    ...
    print(f"[eval-v1] Device: {device}")
    print(f"[eval-v1] Running on {len(test_entries)} images...")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="OCR eval: 10% padding on detector crop")
    ...
    print("\n" + "=" * 60)
    print("  PADDING-V1 OCR EVALUATION (10% padding)")
    print("=" * 60)
    ...
    out_path = PROJECT_ROOT / "data" / "processed" / "eval-padding-v1.json"
