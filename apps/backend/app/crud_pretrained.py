...
def get_jobs_summary(db: Session) -> dict[str, int]:
    total = int(db.execute(select(func.count()).select_from(PretrainedJob)).scalar_one())
    queued = int(
        db.execute(select(func.count()).select_from(PretrainedJob).where(PretrainedJob.status == "queued")).scalar_one()
    )
    running = int(
        db.execute(select(func.count()).select_from(PretrainedJob).where(PretrainedJob.status == "running")).scalar_one()
    )
    success = int(
        db.execute(select(func.count()).select_from(PretrainedJob).where(PretrainedJob.status == "success")).scalar_one()
    )
    failed = int(
        db.execute(select(func.count()).select_from(PretrainedJob).where(PretrainedJob.status == "failed")).scalar_one()
    )
    return {
        "total": total,
        "queued": queued,
